...
// type model, including the @pii tag that the checker enforces.
package value
...
// Both propagate through every expression and both must be cleared
// separately before a sink: mask(x) clears PII, ?? clears Optional, and
// everything else preserves both.
//
// Deidentified is different: it replaces Kind rather than tagging it.
// Inner holds the cell's declared type before encryption and is set only
// when Kind == Deidentified; such a Type's own Optional and PII are
// always false, since optionality is discharged at ingest and
// @pii/@deidentify are mutually exclusive. Inner is a pointer so Type
// stays a fixed-size, comparable value for every other Kind, which never
// sets it.
type Type struct {
	Kind     Kind
	Optional bool
...
// Absence is a well-typed value, not a failure and not a null: Coerce
// returns it for a missing column or an empty cell against an Optional
// Type, and it is never Go's nil.
//
// Kind records the field's declared scalar type, so ??'s decimal-literal
// promotion can still apply when the left operand is absent and there's
// no real DecimalValue to inspect. A propagated Absent (built in
// internal/eval, not by Coerce) leaves Kind unset.
type Absent struct {
	Kind Kind
}

// MarshalJSON renders Absent as JSON null. That's a sink-format detail;
// Sift itself has no null at the language level.
func (Absent) MarshalJSON() ([]byte, error) {
	return []byte("null"), nil
}

// OrderedValue is implemented by a struct-backed Kind's runtime value
// whose Go == doesn't correctly compare the value it denotes. DateValue,
// DecimalValue, and DateTimeValue all wrap a struct (time.Time, or
// decimal.Decimal's own *big.Int underneath) that bare == would compare
// by internal representation rather than by the value denoted.
// CompareValue follows time.Time.Compare and decimal.Decimal.Cmp's own
// -1/0/1 contract, so internal/eval's LT/GT/LE/GE/EQ/NE dispatch shares
// one path across every struct-backed Kind instead of a hand-copied
// branch per type.
...
// timezone. It wraps time.Time rather than aliasing it (`type DateValue
// time.Time`, not `= time.Time`) so a DateValue never silently inherits
// every time.Time method — most of them (timezone conversion,
// Now()-adjacent constructors) don't make sense for a date-only value.
// Converting back for comparison is an explicit time.Time(d) at the call
// site.
type DateValue time.Time

// String renders a DateValue as ISO-8601, the canonical rendering,
...
// date-only value never had.
func (d DateValue) MarshalJSON() ([]byte, error) {
	return json.Marshal(d.String())
}

// CompareValue implements OrderedValue via time.Time.Compare.
...
// second, with no timezone or offset at all. It wraps time.Time rather
// than aliasing it for the same reasons DateValue does: a custom
// MarshalJSON rendering the canonical layout below, and no time.Time
// method leaking in unasked.
type DateTimeValue time.Time

// String renders a DateTimeValue as ISO-8601 with a time component and
// no zone suffix.
...
// DecimalValue is an exact-arithmetic scalar. It wraps decimal.Decimal
// rather than aliasing it because decimal.Decimal's own
// String()/MarshalJSON silently strip trailing zeros ("5.00" renders as
// "5") even though the value's internal exponent still remembers the
// original scale: NewFromString("5.00").Exponent() is -2, but .String()
// drops it anyway. Sift's contract is that what you parse is what comes
// back out, so DecimalValue's own String()/MarshalJSON render at the
// value's remembered scale instead, via StringFixed.
...
// shares time.Time.Compare's own -1/0/1 contract.
...
// enforce that sinks reject unmasked PII. Checking in declared order
// keeps the field named in the error message deterministic.
...
// order, and whether one was found. It mirrors FirstPII: the checker
// calls this once per sink to reject a still-optional field the same way
// it rejects unmasked PII.
...
// order, and whether one was found. A source constructor calls this once
// to require the key: kwarg before reading any row -- the same shape as a
// missing required column, unlike FirstPII/FirstOptional, which a sink
// calls instead.
...
// String renders the schema the way a source declares it:
// { name: string, age: int }. --emit-schema just calls this.
...
// condition that was false, or a source cell that wouldn't coerce to its
// declared type. It never duplicates Provenance: the Row it rides on
// already carries source, ordinal, and offset.
type Failure struct {
...
// it. Only the driver, at the end of the chain, disposes of a failed row
// per the program's error policy.
type Row struct {
