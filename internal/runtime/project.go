...
// Select keeps only the named columns, in the order named
// (design-improvements.md §1). The checker has already validated every
// name exists in the input schema, so Next never needs to guard against
// a missing key — a lookup miss just means the column happens to be
// absent on this particular row's Fields map (can't happen for a
// healthy row, since the checker's schema and the row's actual fields
...
func (s *Select) Next() (value.Row, bool) {
	...
	// A failed row is opaque (design-errors.md §2.2): pass it through
	// rather than projecting its (possibly suspect) fields.
	if row.Fail != nil {
		return row, true
	}
