...
// format needs. This is a plain struct rather than map[string]any: every
// format needs a path plus a declared schema, so a typed struct is
// simpler and catches typos at compile time. It may grow fields for
// format-specific options (e.g. a CSV delimiter) without needing a
...
type SourceOptions struct {
	...
	// xlsx's "sheet"/"header_row"), decoded from
	...
	Opts map[string]any
	// Columns holds the optional columns kwarg: schema field name to the
	// raw header string to match instead
	// of the field's own identifier. Nil for a source with no columns
	// kwarg. Only csv and xlsx look here.
	Columns map[string]string
	// DateFormats holds the optional formats kwarg: a date-typed schema
	// field name to the Go reference-layout string to
	...
	// variable name, e.g. "SIFT_DEIDENTIFY_KEY"
...
// Adding a format never touches the lexer, parser, checker, or executor,
