...
// RequireDeidentifyKey raises a construction error before any row is
// read, the same shape as a missing required column: a schema with a
// @deidentify field but no key: kwarg (envVar == "") is an error naming
// the field; otherwise the kwarg's environment
...
// and decodes it into the 32-byte AES-256 key. A missing variable, an
// undecodable value, or a decoded length other than
...
// decodeKey tries hex first, then standard base64; either encoding is
// accepted. Hex's alphabet is a strict subset of base64's, so
