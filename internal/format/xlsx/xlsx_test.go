...
// TestXLSXSourceDeidentifyMissingKeyIsConstructionError mirrors csv's
// own "before any row is read" rule: a schema with a @deidentify field
// but no key kwarg fails NewXLSXSource itself.
func TestXLSXSourceDeidentifyMissingKeyIsConstructionError(t *testing.T) {
	path := writeXLSXFixture(t, "Sheet1", [][]string{
		{"name", "email"},
		{"Ada", "ada@example.com"},
	})
	...
	if err == nil {
		t.Fatal("NewXLSXSource succeeded, want a construction error")
	}
